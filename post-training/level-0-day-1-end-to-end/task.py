...
INSTRUCTION = (
    ...
    "name (string), age (integer), department (string), "
    "start_date (string, YYYY-MM-DD), salary (integer). "
...
_TEMPLATES = [
    ...
    "row -> {name} | {salary} USD | {department} | age {age} | start {start_date}",
]


def render_row(record: dict, rng: random.Random) -> str:
    """Render a ground-truth record as one messy input line."""
    return rng.choice(_TEMPLATES).format(**record)
